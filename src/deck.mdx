...
<RegexCompare texts={["x  = x + 1", "x = x + 10", "abc =   abc *    10"]} shouldNotMatchTexts={["x = y + 1"]} answerPattern={"(\\w+)\\s*=\\s*\\1\\s*(\\S)+\\s*(\\w+)"} replaceText={""} replaceTextAnswer={"$1 = $1 $2 $3"} />
...
<RegexCompare texts={["x  = x + 1", "x = x + 10", "abc =   abc *    10"]} shouldNotMatchTexts={["x = y + 1"]} answerPattern={"(?<var>\\w+)\\s*=\\s*\\k<var>\\s*(?<symbol>\\S)\\s*(?<value>\\w+)"} replaceText={""} replaceTextAnswer={"$<var> = $<var> $<symbol> $<value>"} />
...
TODO - Things to add
- regex flags
- mention different flavors & engines
- add flavor converter script
